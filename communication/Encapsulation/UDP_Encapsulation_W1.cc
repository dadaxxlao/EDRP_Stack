...
int main() {
    ...
    int my_rank = 1; // This node's rank
    UDPCommunicator comm1(addr_map, my_rank);

    char buffer[1024];
    comm1.recv(buffer, sizeof(buffer), 0);
...
std::unordered_map<int, std::pair<std::string, int>> addr_map = {
    ...
    UDPCommunicator comm(addr_map, my_rank);

    char msg[] = "Hello, world!";
    comm.send(msg, sizeof(msg), 1); // Send to rank 1

    char buffer[1024];
    comm.recv(buffer, sizeof(buffer), 1); // Receive from rank 1
