...
static uint32_t gLocalIp = MAKE_IPV4_ADDR(192, 168, 11, 114);
...
static void dxl_arp_table_init(){
	printf("Initialing Arp Table\n");
	struct arp_table *table = arp_table_instance();
	static uint32_t Ip1 = MAKE_IPV4_ADDR(192, 168, 11, 231);
	static uint8_t Ip1ArpMac[RTE_ETHER_ADDR_LEN] = {0x00, 0xE0, 0x4C, 0x6D, 0x75, 0x33};//00-E0-4C-6D-75-33
	static uint32_t Ip2 = MAKE_IPV4_ADDR(192, 168, 11, 115);
	static uint8_t Ip2ArpMac[RTE_ETHER_ADDR_LEN] = {0x00, 0x50, 0x56, 0xAF, 0xD5, 0x7D};//00:50:56:af:d5:7d
	
	struct arp_entry *entry = rte_malloc("arp entry", sizeof(struct arp_entry), 0);
	memset(entry, 0, sizeof(struct arp_entry));
	entry->ip = Ip1;
	rte_memcpy(entry->hwaddr, Ip1ArpMac, RTE_ETHER_ADDR_LEN);
	entry->type = 1;
...
static int udp_server_entry(__attribute__((unused))  void *arg) {
	...
	localaddr.sin_addr.s_addr = inet_addr("192.168.11.114"); // 0.0.0.0
