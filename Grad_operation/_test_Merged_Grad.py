'''
使用随机数来模拟Merged Gradients，然后使用GradHandle类来处理梯度。
这里的代码没有修改为GPU版本，在分布式中应该注意
'''
...
class GradHandle:
    ...
    def process_grads(self):
      ...
      if other_chunks:
          ...
          avg_grads = torch.mean(torch.abs(stacked_chunks), dim=1)
      ...
      if last_chunk is not None:
          last_avg = torch.mean(torch.abs(last_chunk)).item()
          avg_grads_with_index.append((len(self.grad_chunks) - 1, last_avg))

      avg_grads_with_index.sort(key=lambda x: x[1], reverse=True)
