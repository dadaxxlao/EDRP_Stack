...
def run(rank, size):
    ...
    for epoch in range(5):
        ...
        for data, target in train_set:
            ...
            #Grad Operation
            grad_process_start = time.time()#Grad Time Start
            merged_grads = grad_merge.merge_grads()
            grad_handle = GradHandle(merged_grads, chunk_byte = 2**20)
            high_grad_chunks, low_grad_chunks = grad_handle.process_grads()
            split_grads = grad_handle._split_tensor()
            
            #communication
            comm_start = time.time()#communication time start
            avg_split_grads = average_gradients_gradOP(split_grads)
            comm_time += time.time() - comm_start #communication time end
            ...
            grad_handle_time += time.time() - grad_process_start #Grad Time End
        ...
        total_grad_handle_time += grad_handle_time
        
    total_end_time = time.time() - total_start_time  # 总运行时间计时结束
    print("Total time:", total_end_time)
    print("Total Grad Handle Time:", total_grad_handle_time - total_comm_time)
