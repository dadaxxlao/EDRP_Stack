...
class Net(nn.Module):
    def __init__(self):
        ...
        # Replace the last fc layer for CIFAR10
        num_ftrs = self.model.classifier[6].in_features
        self.model.classifier[6] = nn.Linear(num_ftrs, 10)  # CIFAR10 has 10 classes
...
def get_test_loader():
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Resize to fit AlexNet input size
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
...
#            data, target = Variable(data.cuda(rank)), Variable(target.cuda(rank))
        ...
        print('Rank ',
              ...
              'Commtime per epoch: ', total_comm_time/(comp_end_time - comp_start_time))
